...
class ResumeAnalysisService {
  ...
  buildScoringAnchors() {
    return [
      '【评分校准参考框架】',
      '评分原则：以积极视角挖掘候选人亮点，而非挑剔短板。每个维度的起评分应为该档位区间的中间值。',
      '例如教育背景：本科专业对口不是"11分起步"，而是"13-14分起步"，有亮点再加分。',
      '请基于以下通用评分标尺进行各维度打分：',
      '',
      '教育背景 (0-20分):',
      '- 16-20分：名校硕博，专业与岗位高度对口，学术背景突出',
      '- 12-16分：本科及以上学历，专业基本对口或有相关辅修/第二专业',
      '- 6-11分：大专学历或专业部分相关，可通过经验弥补',
      '- 0-5分：学历偏低或专业与岗位完全不符',
      '',
      '工作经历 (0-20分):',
      '- 16-20分：3年以上高度相关工作经验，有行业知名企业背景，职级较高',
      '- 12-16分：1-3年相关经验或实习经历丰富，有明确的职业成长路径',
      '- 6-11分：有工作经验但行业/岗位相关性一般，或为初级岗位经验',
      '- 0-5分：缺乏相关工作经验，或无正式工作经历',
      '',
      '项目经历 (0-30分):',
      '- 24-30分：多个高质量项目，技术深度与广度俱佳，有可量化的成果与影响力',
      '- 18-24分：有2个以上相关项目，项目描述详实，角色与贡献清晰',
      '- 10-17分：有1-2个项目经历，但描述较简单，角色不够清晰',
      '- 0-9分：无明显项目经历或项目与岗位无关',
      '',
      '技能匹配 (0-25分):',
      '- 20-25分：核心技能全部覆盖，且有超出岗位要求的额外技能或证书',
      '- 15-20分：覆盖大部分核心技能（≥60%），有1-2项突出技能',
      '- 8-14分：覆盖部分核心技能（30%-60%），关键技能有缺失',
      '- 0-7分：技能与岗位要求匹配度低（<30%），或技能描述模糊',
      '',
      '表达完整性 (0-5分):',
      ...
      '',
      '【校准基准示例】以下为三个不同档次的简历评分参考，请据此校准你的评分尺度：',
      '',
      '【强简历参考 ~92分】名校硕士，专业高度对口，3段高质量相关实习/项目，技能覆盖岗位全部核心要求，',
      '有可量化成果(如发论文/竞赛获奖/开源贡献)，简历结构完整逻辑清晰。',
      '→ education:17-20, work:16-19, project:25-29, skill:21-25, expression:4-5, totalScore:88-98',
      '',
      '【中等简历参考 ~78分】本科学历专业基本对口，1-2段相关经历，覆盖60%核心技能，',
      '项目描述较清晰但缺少量化成果，简历基本完整。',
      '→ education:13-16, work:12-15, project:18-22, skill:15-20, expression:3-4, totalScore:73-84',
      '',
      '【弱简历参考 ~52分】学历或专业不匹配，缺乏相关经历，技能覆盖<30%，',
      '简历信息简略或有大段空白，求职意向与岗位不一致。',
      '→ education:5-9, work:3-7, project:5-11, skill:4-9, expression:1-2, totalScore:45-58'
    ].join('\n');
  }
...
` : '';
    ...
    return [
      ...
      '【评分理由】在 summary 中必须简要说明每个维度的评分理由，格式如："教育背景:X分(理由); 工作经历:X分(理由); 项目经历:X分(理由); 技能匹配:X分(理由); 表达完整性:X分(理由)"。',
  ...
  buildSystemPrompt() {
    return [
      '你是一名资深招聘HR和结构化简历评估专家。',
      '你必须严格依据简历原文作答，不要臆造经历、学校、技能或项目。',
      '请从招聘视角给候选人做专业、可落地的评估。以挖掘候选人潜力和亮点为主要目标，评分参照校准基准示例中的较高标准。',
...
` : '';
    ...
    return [
      ...
      '16. 【评分理由】在 summary 中必须简要说明每个维度的评分理由，格式如：”教育背景:X分(理由); 工作经历:X分(理由); 项目经历:X分(理由); 技能匹配:X分(理由); 表达完整性:X分(理由)”。',
      mbtiSection,
