...
// Helper to resolve owner from token (supports both header and query parameter)
function resolveOwnerFromRequest(req) {
  // 优先从 Authorization header 获取 token
  let token = req.headers.authorization?.split(' ')[1];

  // 如果 header 没有，尝试从 URL 查询参数获取（用于 iframe/window.open 等场景）
  if (!token) {
    token = req.query.token;
    console.log('[Auth] 从 URL 参数获取 token:', token ? `${token.substring(0, 20)}...` : 'null');
  } else {
    console.log('[Auth] 从 Header 获取 token:', `${token.substring(0, 20)}...`);
  }

  if (token) {
    try {
      const decoded = jwt.verify(token, JWT_SECRET);
      console.log('[Auth] Token 解码成功:', { id: decoded.id, username: decoded.username });
      return { id: decoded.id, username: decoded.username, email: decoded.email };
    } catch (err) {
      console.log('[Auth] Token 验证失败:', err.message);
    }
  }
  console.log('[Auth] 无有效 token');
  return null;
}
...
function createResumeRouter({ ensureDataFile, scheduleResumeAnalysis } = {}) {
  ...
  router.get('/resume-preview/:id', async (req, res) => {
    try {
      ...
      const html = `
        ...
        <html>
        <head>
          <title>${escapeHtml(candidate.name)} - 简历预览</title>
          <style>
            body {
        ...
        <body>
          <div class="resume-container">
            <div class="header">
              ...
              <p style="color: #666; margin: 5px 0;">${escapeHtml(candidate.position)} | ${escapeHtml(candidate.mbti || 'MBTI待完成')}</p>
            </div>
            ...
            <div class="section">
              ...
              <div class="info-row">
                ...
                <span>${escapeHtml(candidate.mbti || '未完成')}</span>
              </div>
              <div class="info-row">
                <span class="info-label">提交时间:</span>
                <span>${escapeHtml(candidate.submitTime)}</span>
              </div>
            </div>
            ...
            <div class="section">
              ...
              <div class="info-row">
                ...
                <span>${escapeHtml(candidate.resumeFileName)}</span>
              </div>
              <div class="info-row">
            ...
            <div class="section">
              ...
              <div class="skills">
                ${candidate.analysisDetails.resumeAnalysis.skillMatches.map(match =>
                  `<span class="skill-tag">${escapeHtml(match.skill)} (${match.type === 'core' ? '核心' : '商业'})</span>`
                ).join('')}
              </div>
            </div>
            ...
            <div class="analysis-section">
              ...
              <ul>
                ${candidate.analysisDetails.resumeAnalysis.highlights.map(highlight =>
                  `<li class="highlight">${escapeHtml(highlight)}</li>`
                ).join('')}
              </ul>
            </div>
            ` : ''}
            ...
            <div class="section">
              ...
              <div class="info-row">
                ...
                <span style="color: #1890ff; font-weight: bold;">${escapeHtml(String(candidate.matchScore))}%</span>
              </div>
              <div class="info-row">
  ...
  router.get('/resume-file/:id', async (req, res) => {
    try {
      console.log('[Resume File] ========== 请求开始 ==========');
      console.log('[Resume File] 请求 URL:', req.originalUrl);
      console.log('[Resume File] 请求参数:', req.params);
      console.log('[Resume File] Query 参数:', req.query);
      console.log('[Resume File] Headers:', JSON.stringify(req.headers.authorization || '无 Authorization header', null, 2));

      const candidateId = parseInt(req.params.id);

      const owner = resolveOwnerFromRequest(req);
      console.log('[Resume File] owner 结果:', owner);
      if (!owner) {
        console.log('[Resume File] 认证失败，返回 401');
      ...
      if (!candidate) {
        console.log('[Resume File] 候选人不存在');
        return res.status(404).send('候选人不存在');
      }

      console.log('[Resume File] 候选人信息:', {
        id: candidate.id,
        name: candidate.name,
        ownerUserId: candidate.ownerUserId,
        hasBuffer: !!candidate.resumeFileBuffer,
        hasPath: !!candidate.resumeFilePath,
        fileName: candidate.resumeFileName
      });

      // Verify ownership (使用 ownerUserId 而不是 owner_user_id)
      if (candidate.ownerUserId !== owner.id) {
        console.log('[Resume File] 所有权验证失败:', candidate.ownerUserId, '!==', owner.id);
      ...
      if (!candidate.resumeFileBuffer && !candidate.resumeFilePath) {
        console.log('[Resume File] 简历文件不存在');
      ...
      console.log('[Resume File] 文件扩展名:', ext);
      ...
      console.log('[Resume File] Content-Type:', contentType);
