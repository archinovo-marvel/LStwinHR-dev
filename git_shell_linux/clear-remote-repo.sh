...
new_temp_repo_path() {
    echo "/tmp/lstwinhr-clear-$(date +%s%N)"
}
...
resolve_target_branch() {
    ...
    for i in "${!remote_branches[@]}"; do
        ...
        mark=""
        if [ "$branch_name" = "$current_branch" ]; then mark=" (当前本地分支)"; fi
        echo -e "  $((i+1)). $branch_name$mark"
    done
    ...
    while true; do
        ...
        normalized=$(echo "$input_value" | tr -d '[:space:]')
        
        if [[ "$normalized" =~ ^[0-9]+$ ]]; then
            index=$normalized
            if [ "$index" -ge 1 ] && [ "$index" -le ${#remote_branches[@]} ]; then
                BRANCH_RESULT="${remote_branches[$((index-1))]}"
                return
            fi
        fi
...
resolve_target_branch "${remote_branches[@]}"
...
cd "$ORIGINAL_DIR"
if [ -n "$temp_repo" ] && [ -d "$temp_repo" ] && [ "$KEEP_TEMP" != true ]; then
    rm -rf "$temp_repo"
fi
